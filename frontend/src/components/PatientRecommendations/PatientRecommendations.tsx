...
const PatientRecommendations: FC<IPatientRecommendationsProps> = ({recommendations}) => {
  ...
  return (
    <Box>
      <Typography fontSize={20} marginBottom={1}>Рекомендации</Typography>
      {sortByDate(recommendations).map(recommendation => <Accordion key={recommendation.id}>
      <AccordionSummary expandIcon={<ExpandMoreIcon/>}>
        <Typography>Рекомендация от {formatDate(recommendation.date)}</Typography>
      </AccordionSummary>
      <AccordionDetails>
        <Typography>
          {recommendation.text}
